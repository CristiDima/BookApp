...
@Component({
  ...
  export class UpdateBookComponent implements OnInit {
    ...
    constructor(private _bookService: BookService, private _apiRequest: APIRequestService, private _pathRequest: PathRequestService,
                ...
                private _genreService: GenreService, private _authorService: AuthorService, private cd: ChangeDetectorRef) {
    }
    ...
    private onSetForm() {
        ...
        this.selectedBook.genres.forEach(genre => {
            const tempGenre: Genre = this._genreService.genres.filter(el => el.name === genre.name)[0];
            if (tempGenre) {
                tempGenre.uiSelected = true;
            }
        });
        this.selectedBook.authors.forEach(author => {
          const tempAuthor: Author = this._authorService.authors.filter(el => el.name === author.name)[0];
          if (tempAuthor) {
    ...
    private updateBookRequest(book: Book): void {
        this.spinner.show();
        const succesMsg: string = 'The book: `' + book.name + '` was updated successfully';
        const errorMsg: string = 'An error occured. Please try again.'
        this._apiRequest.requst('PUT', this._pathRequest.bookPath, book).subscribe((responseData: Book) => {
